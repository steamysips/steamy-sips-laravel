            ...
            <form method="post" action="{{ route('stores.update', $store->store_id) }}">
                ...
                <div class="form-group">
                    ...
                    <input type="text" class="form-control" name="latitude"
                        value="{{ $store->coordinate ? $store->coordinate['latitude'] : '' }}" />
                </div>
                <div class="form-group">
                    <label for="longitude">Longitude</label>
                    <input type="text" class="form-control" name="longitude"
                        value="{{ $store->coordinate ? $store->coordinate['longitude'] : '' }}" />
                </div>
                <div class="form-group">
                    <label for="districts">District</label>
                    <select class="form-control" name="districts">
                        @foreach ($districts as $district)
                            <option value="{{ $district }}" {{ $store->district == $district ? 'selected' : '' }}>
                                {{ $district }}</option>
                        @endforeach
                    </select>
                </div>
